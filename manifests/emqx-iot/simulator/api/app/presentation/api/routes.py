...
@router.get("/devices")
async def devices(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
    return await telemetry.list_devices()


@router.get("/events")
async def events(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
    return await telemetry.list_events()


@router.get("/summary")
async def summary(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
...
@router.get("/alerts")
async def alerts(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
    return await telemetry.list_alerts()


@router.get("/gold/fleet-summary")
async def gold_fleet_summary(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
    return await telemetry.get_gold_fleet_summary()


@router.get("/gold/zones")
async def gold_zones(telemetry: TelemetryUseCase = Depends(get_telemetry_use_case)):
